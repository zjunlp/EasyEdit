...
class BaseVectorApplier:
    ...
    def _process_input(self, item) -> dict:
        """
        Build model-ready inputs for one generation item, adapting to modality.
        """
        ...
        if hasattr(self.model, 'prompt'):
            input_text = f"{self.model.prompt} {input_text}"
        ...
        proc_kwargs = dict(
            ...
            padding="longest", truncation=True, max_length=self.config.max_length,
        )
        if image is not None:
        ...
        for generation_data_name in generation_data_names:
            ...
            if self.model.VLLM_model is not None:
                ...
                for item in tqdm(dataset, desc=f"Evaluating dataset {generation_data_name}"):
                    ...
                    current_preds = []
                    current_output = []
                    inputs = self._process_input(item)
                    num_responses = self.config.get('num_responses', 1)
                    for j in range(num_responses):
                        if num_responses > 1:
                            set_seed(j)
