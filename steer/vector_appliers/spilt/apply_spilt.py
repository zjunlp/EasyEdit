...
def apply_spilt(hparams: ApplySpiltHyperParams, pipline=None, vector=None):
    ...
    if pipline is None:
        ...
        model = pipline
    print('Apply spilt to model: {}'.format(hparams.model_name_or_path))
    # Reset only spilt activations for specified layers
    reset_spilt_layers(model, hparams.layers)
    ...
    for layer, multiplier in zip(layers, multipliers):
        ...
        model.set_intervention(layer, intervention, "spilt")
