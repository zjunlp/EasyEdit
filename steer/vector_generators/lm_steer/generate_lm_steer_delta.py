...
def generate_lm_steer_delta(hparams: LmSteerHyperParams, dataset, model=None, dataset_name=None):
    ...
    from ...datasets import load_lm_steer_dataset, lm_steer_collate
    ...
    # Modality is carried by the wrapper: text wrappers expose processor=None, multimodal
    # wrappers expose a real image processor. Everything below adapts on this single flag.
    processor = getattr(model, 'processor', None)

    train_data = load_lm_steer_dataset(
        dataset, hparams.subset, tokenizer, hparams.system_prompt, hparams.use_chat_template,
        processor=processor,
    )
    dataloader = DataLoader(
        train_data, batch_size=hparams.batch_size, shuffle=True,
        collate_fn=lm_steer_collate,  # keep optional PIL images as a list
    )
    ...
    for step_i in pbar:
        ...
        model_inputs = _build_model_inputs(batch_text, batch_image, tokenizer, processor, hparams, device)
...
def _build_model_inputs(batch_text, batch_image, tokenizer, processor, hparams, device):
    """
    Tokenize a batch into model kwargs, adapting to modality.
    """
    if processor is not None and batch_image is not None:
        proc = processor(
            text=batch_text,
            images=batch_image,
            return_tensors="pt",
        ...
        tokenized = tokenizer(
            batch_text, padding=True, max_length=hparams.max_length, truncation=True,
