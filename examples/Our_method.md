...
- LLM control methods (local weight edits, LoRA adaptation, and activation steering) are often studied in isolation, limiting systematic comparison. We unify them as control-signal-induced dynamic weight updates and introduce a Preference–Utility evaluation that measures concept preference and generation utility on a shared log-odds scale. Experiments reveal a consistent trade-off: stronger control increases preference but harms utility, which we explain via an activation-manifold view. We further propose a new steering method that improves preference while better preserving utility.

- The implementation focuses on generating steering vectors from training data and applying them during generation to achieve desired behavioral changes while maintaining model coherence and performance.
...
conda create -n plus python=3.10
conda activate plus
...
#### 2. Run the script [run_our.py](../run_our.py) using the following line of code:
 
    bash examples/run_our.sh
