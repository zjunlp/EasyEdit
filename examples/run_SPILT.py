#!/usr/bin/env python3
"""
spilt Experiment Runner

This script runs steering vector generation and application experiments
for different datasets (axbench, psychopathy, powerseeking), methods (caa, reps, sft, spilt),
...
VALID_METHODS: List[str] = ["caa", "reps", "sft", "spilt"]
...
def _resolve_generate_hparam(
    ...
    Some folders use legacy filenames (e.g. caa uses generate_caa.yaml, spilt uses generate_spilt_weight.yaml).
    """
    base = f"{base_dir}/hparams/Steer/experiment_hparams/spilt_experiment/{dataset}/{model_name}/{method}"
    candidates: List[str] = [
...
def run_vector_generation(
    ...
    Args:
        ...
        method: Method name (caa, reps, sft, spilt)
    ...
    if not train_hparam:
        expected = f"{base_dir}/hparams/Steer/experiment_hparams/spilt_experiment/{dataset}/{model_name}/{method}/..."
    ...
    if dataset == "axbench":
        ...
        cmd = [
            ...
            f"steer_train_hparam_paths=[{train_hparam}]",
            # Keep method-specific isolation to avoid collisions (e.g., spilt vs reps)
            f"steer_vector_output_dirs=[vectors/{model_name}/{method}]",
        ]
...
def run_loss_calculation(
    ...
    train_hparam = os.path.join(
        base_dir,
        f"hparams/Steer/experiment_hparams/spilt_experiment/{dataset}/{model_name}/sft/generate_sft_loss.yaml"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run spilt experiments for steering vector generation and application",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate vectors for axbench dataset using reps method with vector intervention
  python run_spilt.py --dataset axbench --method reps --model_name gemma-2-9b-it --intervention_method vector --mode generate

  # Apply vectors for axbench dataset using sft method with lora intervention
  python run_spilt.py --dataset axbench --method sft --model_name gemma-2-9b-it --intervention_method lora --mode apply --multipliers 1.0 2.0

  # Run both generation and application
  python run_spilt.py --dataset axbench --method reps --model_name gemma-2-9b-it --intervention_method vector --mode both

  # Calculate loss for psychopathy dataset
  python run_spilt.py --dataset psychopathy --method spilt --model_name gemma-2-9b-it --intervention_method local_weight --mode loss --multipliers 0.2
        """
    )
