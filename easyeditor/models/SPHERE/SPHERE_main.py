...
def sparse_projection(A, B, eta=0.5, alpha=0.5):
    """
    Docstring for sparse_projection
    
    :param A: Weight matrix for the current model
    :param B: Calculated update matrix
    :param eta: cumulative ratio
    :param alpha: suppression strength

    returns: (1) Projected update matrix B_proj; (2) Soft projection matrix P_soft; (3) Eigenvectors U used in the projection
    """
...
def upd_matrix_match_shape(matrix: torch.Tensor, shape: torch.Size) -> torch.Tensor:
    ...
    if matrix.shape == shape:
        ...
        raise ValueError(
            "Update matrix computed by SPHERE does not match original weight shape. "
