...
def upd_matrix_match_shape(matrix: torch.Tensor, shape: torch.Size) -> torch.Tensor:
    ...
    if matrix.shape == shape:
        ...
        raise ValueError(
            "Update matrix computed by CORE does not match original weight shape. "
...
def get_context_templates(
    ...
    if CONTEXT_TEMPLATES_CACHE is None:
        ...
        for i in range(num_prompts):
            if i < remainder:
                allocations.append(base_allocation + 1)
            else:
        ...
        for length, _ in gen_configs:
            templates = []
            for prompt, n_gen in zip(default_prompts, allocations):
                if n_gen > 0:  
