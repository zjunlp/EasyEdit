...
class LlamaRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if not self.weight.device == hidden_states.device:
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    ...
    if cos.device != gather_indices.device:
        cos = torch.gather(cos.repeat(gather_indices.shape[0], 1, 1, 1), 2, gather_indices.to(cos.device))
        sin = torch.gather(sin.repeat(gather_indices.shape[0], 1, 1, 1), 2, gather_indices.to(cos.device))
    else:
